...
import type { QAResponse } from "@ebs/ground-truth-schema";
...
export function candidateInputForQa(input: {
  ...
  const modelField =
    input.qa.suggested_writeback?.field_key ?? input.qa.target_field ?? null;
  const fieldKey =
    (isStructuredFieldKey(input.requestedTargetField)
  ...
  return {
    ...
    sourceBlockIds: input.qa.source_block_refs.length
      ? input.qa.source_block_refs
      : input.evidenceBlockIds,
  };
}
